...
def maximum_sum_path(arr)

  loop_index = arr.length-1
  output = arr.last

  while loop_index > 0
    output = compare_neighbor(output)

    output = output.each_with_index.map {|value, index|
    value += arr[loop_index-1][index]}
