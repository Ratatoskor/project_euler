...
def abundant_nums_generator(num)
  abundant_nums =[]
  (1..num).each do |x|
    # Needs more optimization, but any multiples of abundant numbers are also abundant
...
Abundant_nums = abundant_nums_generator(20500)

def is_sum_of_abundants?(num)

  Abundant_nums.each do |abun_num|
    return false if num < abun_num
...
sum_of_abundants(20500)


#SLOW - Any way to speed it up?
